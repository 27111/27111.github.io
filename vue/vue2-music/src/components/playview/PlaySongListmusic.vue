<template>
  <li class="home-song">
    <template v-if="music.song">
      ...
      <span
        class="f-btn"
        v-if="songId !== music.id || playing === false"
      ></span>
      <div
        class="playing-statue"
        v-if="songId === music.id && playing === true"
      >
    ...
    <template v-else>
      <div class="f-songName f-active">{{ music.name }}</div>
      <div class="f-singer f-active">
        <i class="f-icon" v-show="showIcon(music.sq)"></i>
        <!-- v-show="showIcon(music.song.sqMusic)" -->
